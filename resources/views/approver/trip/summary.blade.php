...
<main id="main" class="main">
    <section class="section">
        ...
        <div class="col-lg-12 mt-3">
            <center>
                @php
                $userlist= App\Models\UserDetail::where('user_id',$trip->user_id)->where('repostingto', auth()->user()->emp_id)->first();
                $reporting = ($userlist) ? $userlist->repostingto : 0;
                @endphp

                @if (($trip->user_id != auth()->user()->id) || (auth()->user()->emp_id == $reporting))

                @if ($trip->status == 'pending')
                <a href="{{ route('approver.approve',$trip->id) }}" class="btnStatus btn btn-success  text-white"><i class="bi bi-check-circle-fill"></i> Approve</a>
                <a href="{{ route('approver.reject',$trip->id) }}" class="btnStatus btn btn-danger text-white"><i class="bi bi-x-circle-fill"></i> Reject</a>
